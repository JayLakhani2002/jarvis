...
## What we're building
Jarvis is a personal AI-operations system: a set of unattended launchd jobs plus a Telegram bot that read/write an Obsidian vault (`~/Documents/J's AI Brain`), and a new desktop app (Jarvis v2) that puts a voice-first UI on top of that same backend. Hard rule throughout: draft-only — nothing auto-sends, auto-deploys, or auto-spends without the operator ratifying it.

## Done — v1 backend (shipped, v1 → v1.8.1; restored 2026-08-12 after a repo-move outage)
12 launchd jobs, all live in `bin/` + `launchd/`:
- `telegrambot`, `nightshift` (**paused** by the operator), `watchdog`, `briefingpush`, `vaultsnapshot`, `dailysync`, `weeklysync`, `marketradar`, `voiceserver`, `dashboard`, `ragindex`, `emailtriage`, `networth`
...
- **Phase 2 has still never been run live** — `config/voice-app.conf` was never created; blocked on the operator signing up for Deepgram + ElevenLabs free tiers (recipe ready in vault `06 Company/Drafts/Jarvis Voice v2 — Setup.md`, paths corrected)

## Open / not started — Jarvis v2 Phases 3–7
From `JARVIS-V2-PLAN.md`, waiting on the operator's explicit "go" each time (weekend-sized, scheduled around the one-track priority order (deadline-ranked workstreams)). Confirmed: 7 of 9 left-nav items (Tasks, Schedules, Tools, Pipelines, Content, Agents, Businesses) are `disabled` stubs with no backing view yet.
- **Phase 3 — Router:** classify every request (personal / routine / reminder / project) and route to the right skill/agent
- **Phase 4 — Company in the app:** agent chat pages + business dashboard (venture stage/progress) in the UI
...
- **`ragindex` fails because Ollama isn't installed** on this Mac at all (not a code bug) — the operator's hands if he wants vault RAG search back
- **Phase 2 voice round-trip unverified live** — code is complete and unit-tested but has never actually spoken to Deepgram/ElevenLabs; blocked on the operator's API key signups
- **Night shift remains paused** — parked until the operator picks the project it should run on
- **iOS Shortcuts** (voice capture, health logging, one-time Calendar TCC grant for the 📅 briefing line) — blocked on the operator's hands, not code
- Root also has two uncommitted planning docs (`JARVIS-V2-PLAN.md`, `JARVIS-V2-PRD.md`) plus `prompt.md`, all still untracked

## Immediate next decision
Commit the `app/` + `jarvis-orb/` work (Phases 0–2), then either: the operator does the voice signups so Phase 2 can be verified live end-to-end, or the operator says "go" on Phase 3 (Router) in parallel. Separately: the `vaultsnapshot` silent-failure discovery needs its own look before it's trusted again as a backup.
